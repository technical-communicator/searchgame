...
const VENUES = [
  {
    ...
    vibe: "ARTSY",
    icons: ["☕", "✨", "☕"],
    description: "Panamanian-inspired hidden gem in South Eola. Vibrant, magical décor and warm service. World-class Geisha coffee."
  },
  {
    name: "WILL'S PUB",
    file: "wills.png",
    vibe: "INDIE",
    icons: ["♪",  "♫",  "★"],
    description: "Iconic dive bar since 1995. Heart of Orlando's indie music scene. Live performances & craft drinks."
  },
  {
    name: "SYZN THRIFT",
    file: "syzn.png",
    vibe: "EDGY",
    icons: ["✦",  "⚡",  "★"],
    description: "Curated vintage & alternative fashion near UCF. Affordable finds and unique one-of-a-kind pieces."
  },
  {
    name: "FUNKY'S",
    file: "funkys.png",
    vibe: "INDIE",
    icons: ["✦",  "♻",  "✨"],
    description: "Milk District's Best Vintage Store. Quality vintage clothing, retro games, and collectibles."
  },
];
...
function drawIntro() {
  ...
  drawPixelBorder(20, 100, W - 40, 300, 4, color(255, 220, 100));
  ...
  textSize(14);
  fill(180, 160, 220);
  text("~ SINGLE SESSION ADVENTURE ~", W / 2, 330);

  let bob = sin(frameCount * 0.05) * 8;
  drawSprite(searchImg, W / 2, H / 2 + 80 + bob, 200);
...
function drawMorning() {
  ...
  drawPixelBorder(10, 10, W - 20, 80, 3, color(120, 90, 160));
  ...
  textSize(18);
  textStyle(BOLD);
  text("MORNING ROUTINE", W / 2, 45);
  textStyle(NORMAL);

  if (selectedItem === null) {
    let bob = sin(frameCount * 0.05) * 8;
    drawSprite(searchImg, W / 2, 180 + bob, 120);

    fill(50, 30, 70);
    textAlign(CENTER, TOP);
    textSize(13);
    text("SELECT AN ITEM", W / 2, 300);
    ...
    ROOM_ITEMS.forEach((item, i) => {
      ...
      textAlign(CENTER, CENTER);
      textSize(64);
      text(item.emoji, item.x, item.y - 5);
...
function drawMap() {
  ...
  if (mapScroll > 200) {
    ...
    drawPixelChatBubble(W / 2, H / 2 - 40, ">> " + v.name, 180, 100);
    if (mapScroll > 260) {
      fill(180, 150, 220);
...
function drawVenue() {
  ...
  drawSprite(mascotImgs[v.file], mascotX, 120 + bob, 180);
  ...
  textSize(13);
  fill(140, 100, 180);
  text("[ " + v.vibe + " VIBES ]", W / 2, 52);

  fill(70, 45, 110);
  textSize(11);
  textAlign(CENTER, TOP);
  text(v.description, 18, 75, W - 36, 95);
...
function drawEndcard() {
  ...
  drawPixelBorder(30, 80, W - 60, 300, 4, color(255, 220, 100));
  ...
  let bob = sin(frameCount * 0.05) * 8;
  drawSprite(searchImg, W / 2, H / 2 - 40 + bob, 150);

  textSize(16);
  fill(180, 160, 220);
  text("TODAY'S VIBE:", W / 2, H / 2 + 90);

  textSize(40);
  fill(255, 220, 100);
  textStyle(BOLD);
  text(personalityLabel, W / 2, H / 2 + 150);
  textStyle(NORMAL);
  ...
  textSize(12);
  fill(160, 140, 200);
  text(eLabel + "  ::  " + sLabel, W / 2, H / 2 + 200);

  drawPixelButton("PLAY AGAIN", W / 2, H - 80, 200, 50);
}
...
function resolvePersonality() {
  ...
  return top ? top.toUpperCase() : "CASUAL";
}
...
function drawPixelButton(label, cx, cy, bw, bh) {
...
function drawPixelChatBubble(cx, cy, text, w, h) {
  fill(255, 245, 255);
  stroke(140, 100, 180);
  strokeWeight(3);
